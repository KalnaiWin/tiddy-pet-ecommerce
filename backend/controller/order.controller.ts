...
export const getOrderForShipper = async (req: Request, res: Response) => {
  const shipperId = req.user._id;
  const page = Number(req.query.page) || 1;
  const status = String(req.query.status) || "";
  try {
    if (!shipperId)
      return res.status(404).json({ message: "ShipperId not found" });
    const result = await OrderService.getOrderForShipper(shipperId, page, status);
    return res.status(200).json(result);
  } catch (error) {
    return res.status(500).json({
      message: error instanceof Error ? error.message : "Internal Server Error",
...
export const changeStatusOrder = async (req: Request, res: Response) => {
  ...
  try {
    if (!userId || !orderId)
      return res.status(404).json({ message: "UserId, orderId not found" });
