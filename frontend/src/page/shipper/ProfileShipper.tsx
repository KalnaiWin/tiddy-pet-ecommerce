...
const ProfileShipper = () => {
  ...
  const [selectVehicle, setSelectVehicle] = useState(
    currentUser?.shipper_info.vehicle_type,
  );
  ...
  const [formData, setFormData] = useState({
    ...
    shipper_info: {
      vehicle_type: currentUser?.shipper_info.vehicle_type,
      license_number: currentUser?.shipper_info.license_number,
      verification_status: currentUser?.shipper_info.verification_status,
    },
  });
    ...
    if (currentUser) {
      setFormData({
        ...
        shipper_info: {
          ...
          license_number: currentUser?.shipper_info.license_number || "",
          verification_status:
            currentUser?.shipper_info.verification_status || "",
        },
      });
    }
  }, [currentUser]);
              ...
              <div className="grid xl:grid-cols-2 grid-cols-1 gap-6">
                ...
                <div className="space-y-2">
                  ...
                  <div className="flex gap-2">
                    {["BIKE", "MOTORBIKE", "CAR"].map((type) => (
                      <button
                        key={type}
                        onClick={() => setSelectVehicle(type)}
                        className={`flex-1 py-2.5 px-2 rounded-xl border text-sm font-bold transition-all ${
                          selectVehicle === type
